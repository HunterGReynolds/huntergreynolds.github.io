...
Fortunately, it's pretty easy to get set up and developing for the Gameboy thanks to the [[https://rgbds.gbdev.io/][RGBDS toolchain]]. Their documentation, especially the [[https://gbdev.io/pandocs][PanDocs]]
...
All of the code for this project can be found [[https://github.com/HunterGReynolds/gameboy_hello_world][Here on Github]].

You should get an emulator with a decent debugger and high accuracy. I use [[https://bgb.bircd.org/][the BGB emulator]],
which is a Windows only application, but it's also tested on Wine. If you're looking for a good open-source emulator
with a debugger, [[https://sameboy.github.io/][Sameboy is a good choice]].
...
I'm using a loop to define the RST vectors here, with the names being rst_vector0, rst_vector1, etc. For each one, we
...
See [[https://gbdev.io/pandocs/Interrupt_Sources.html][here]] for a more detailed description of each interrupt. The one
we're interested in here is the vblank interrupt, as this will fire off once per frame, just after the screen is done
being drawn. This is important when we want to access VRAM or OAM (sprites and such). See [[https://gbdev.io/pandocs/Rendering.html#ppu-modes][here]] and [[https:io/pandocs/OAM_DMA_Transfer.html#oam-dma-bus-conflicts)][here]] for explanations of what is accessible
...
[[https://gbdev.io/pandocs/Power_Up_Sequence.html#monochrome-models-dmg0-dmg-mgb][this page]] for an overview of the
...
certain actions and access VRAM. Refer back to [[*Interrupt Handlers][here for more info]].
#+end_quote
...
[[https://rgbds.gbdev.io/][the RGBDS website]]. It's full of DEFs for various registers and flags, so you don't have
...
#+attr_html: :class bgb_screenshot
[[file:bgb.png]]
